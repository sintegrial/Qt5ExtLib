...
void QtPieMenu::insertItem(const QString &text, QObject *receiver,
                           const char *member, QVariant var, int index)
{
    QtPieAction *action = new QtPieAction(text, receiver, member, var);
...
/*!
    ...
    top of the pie and going counter clockwise.
*/
void QtPieMenu::insertItem(const QIcon &icon, QObject *receiver, const char *member, QVariant var, int index)
{
    QtPieAction *action = new QtPieAction(icon, QString::null, receiver, member, var);
...
void QtPieMenu::insertItem(const QIcon &icons, const QString &text,
                           QObject *receiver, const char *member, QVariant var, int index)
{
    QtPieAction *action = new QtPieAction(icons, text, receiver, member, var);
...
void QtPieMenu::paintEvent(QPaintEvent *)
{
    ...
    if (items.size() > 1)
    {
        for (int i = 0; i < items.size(); ++i)
        {
            ...
            {
                grad.setColorAt(1, palette().highlight().color().darker( 70 ) );
                grad.setColorAt(0, palette().highlight().color() );
            }
            else
            {
                grad.setColorAt(1, palette().light().color() );
                grad.setColorAt(0, palette().dark().color() );
            }

            if (shaded)
            {
                grad.setColorAt(1, palette().light().color().darker( 150 ) );
                grad.setColorAt(0, palette().dark().color().darker( 150 ) );
                p.setBrush(grad);
            }
    ...
    QColor light = palette().background().color();
    if (hItem == -1)
    {
        grad.setColorAt(1, palette().highlight().color().darker( 50 ) );
        grad.setColorAt(0, palette().highlight().color() );
    }
    else
    {
    ...
    if (shaded)
    {
        ...
        dark = palette().dark().color().darker( 150 );
        light = palette().light().color().darker( 150 );
    }
    ...
    for (i = 0; i < items.size(); ++i)
    {
        QString text = items.at(i)->text().replace( " ", "\n" );
